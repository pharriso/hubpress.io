...
I have a three node Tower cluster and 49 hosts in my inventory for me to automate against. Here is the available capacity in forks reported by my Tower hosts. 
...
When I launch my job we can see that my playbook is being run on a single Tower node. As I have allocated more forks than I safely have capacity for I am over-allocated on capacity. My other two Tower nodes are just sat there relaxing.
